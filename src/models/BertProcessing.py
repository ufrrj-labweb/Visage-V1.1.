...
class BertProcessing:
    ...
    def upsample_alt(self, features, target, repeat, value):
        features_true = features[target == value]
        features_false = features[target != value]
        target_true = target[target == value]
        target_false = target[target != value]
        ...
        features_list=[]
        target_list=[]
        features_list.append(features_false)
        target_list.append(target_false)
        for i in range(repeat):
            features_list.append(features_true)
            target_list.append(target_true)
            

        features_upsampled = pd.concat(features_list)
        target_upsampled = pd.concat(target_list)
    ...
    def evaluate_alt_upsampled(self,X,y,n_splits=5):
        ...
        for fold, (train, test) in enumerate(cv.split(X, y)):
            ...
            train_data,train_target=self.upsample_alt(train_data,train_target,71, "Medium")
            train_data,train_target=self.upsample_alt(train_data,train_target,13, "High")
            train_data,train_target=self.upsample_alt(train_data,train_target,3, "VeryHigh")
            train_data,train_target=self.upsample_alt(train_data,train_target,240, "Low")
    ...
    def text_processing(self,X,y,n_splits=5):
        ...
        for fold, (train, test) in enumerate(cv.split(X, y)):
            ...
            df_train=pd.DataFrame()
            df_train["Classe de Violência"]=y[train]
            df_train["text"]=X[train]
            df_test=pd.DataFrame()
